...
def article_column(request):
    ...
    if request.method == 'POST':
        ...
        if columns:
            return HttpResponse('2')  # 2代表已存在
        else:
            ArticleColumn.objects.create(user=request.user, column=column_name)
            return HttpResponse('1')  # 1代表创建成功


@login_required(login_url='/account/login/')
@require_POST  # 只接收POST请求
@csrf_exempt
def rename_article_column(request):  # 修改栏目名称
    ...
    if request.method == 'POST':
        ...
        if article_post_form.is_valid():
            ...
            try:
                ...
                if tags:
                    for atag in json.loads(tags):  # 将json格式的数据转换为列表
                        tag = request.user.tag.get(tag=atag)
                        new_article.article_tag.add(tag)
                return HttpResponse('1')  # 发布成功
            except Exception as e:
                return HttpResponse('2')  # 发布失败
        else:
            return HttpResponse('3')  # 数据错误
    else:
...
def article_list(request):
    ...
    try:
        ...
        articles = current_page.object_list  # 得到该页所有的对象列表
    except PageNotAnInteger:  # 页码数值不是整数
        current_page = paginator.page(1)
        articles = current_page.object_list
    except EmptyPage:  # 数值为空
...
def redit_article(request, article_id):
    if request.method == 'GET':
        article_columns = request.user.article_column.all()  # 全部栏目
        article = ArticlePost.objects.get(id=article_id)
