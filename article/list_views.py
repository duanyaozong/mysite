...
def article_content(request, id, slug):
    ...
    total_views = r.incr('article:{}:views'.format(article.id))  # 阅读次数
    r.zincrby('article_ranking', 1, article.id)  # 以步长1自增

    article_ranking = r.zrange('article_ranking', 0, -1, desc=True)[:4]  # 有序集合中排序前4名的对象，按分数来排序
    ...
    # 只能查自己的相似文章，与表结构有关
    article_tags_ids = article.article_tag.values_list('id', flat=True)  # 得到article对象的属性article_tag的id列表
